...
public class Monitor {
	...
	private void updateStats(double x, double u, double rC, double wC, double r, double w, double m) {
		Connection connection = null;
		Statement stmt = null;
		try {
			Class.forName("com.mysql.jdbc.Driver").newInstance();
			...
			connection.setAutoCommit(true);
			stmt = connection.createStatement();
			StatsQuery stats = new StatsQuery(x, u, rC, wC, r, w, m);
			stmt.executeUpdate(stats.getQueryStr());
			LOG.info("Stats: Interval:" + x + ", Queries:" + u + ", Read Queries:" + rC + " Write Queries:" + wC + " Read:" + r + ", Write:" + w + ", Nodes:" + m);
		} catch (Exception e) {
			LOG.error(e.toString());
		} finally {
			if (stmt != null)
	...
	public String readPerformance() {
		...
		int count = 0;
		int totCount = 0;
		int rCount = 0;  // Read Wordload
		int wCount = 0;  // Write Wordload
		double avgRead = 0.0;
		double avgWrite = 0.0;
